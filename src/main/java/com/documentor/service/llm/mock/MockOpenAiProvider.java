...
/**
 * 🎭 MockOpenAiProvider - Mock implementation of OpenAI API provider
 *
 * Simulates OpenAI API responses with deterministic output for testing
...
public class MockOpenAiProvider implements MockLlmProvider {
    ...
    /**
     * Creates a mock OpenAI provider with default model.
     */
    ...
    /**
     * Creates a mock OpenAI provider with specific model.
     *
    ...
    private String generateMockCompletion(final String prompt, final String model) {
        ...
        if (promptLower.contains("code") || promptLower.contains("java")) {
            return """
                    [Mock OpenAI - gpt-3.5-turbo] Here's a Java code example:

                    ```java
                    public class Example {
                        public static void main(String[] args) {
                            System.out.println("Mock response from OpenAI");
                        }
                    }
                    ```
                    """;
        ...
        if (promptLower.contains("document") || promptLower.contains("explain")) {
            return """
                    [Mock OpenAI - gpt-3.5-turbo] Documentation:
        ...
        if (promptLower.contains("fix") || promptLower.contains("error")) {
            return """
                    [Mock OpenAI - gpt-3.5-turbo] Here's a suggested fix:

                    The issue appears to be related to null pointer handling.

                    Suggested solution:
                    1. Add null checks before operations
                    2. Use Optional or defensive programming
                    3. Add proper error handling
                    """;
        }

        return "[Mock OpenAI - " + model + "] Generic response: " +
                "This is a mock response from OpenAI provider. " +
                "Actual API would return real LLM-generated content.";
    }
}
