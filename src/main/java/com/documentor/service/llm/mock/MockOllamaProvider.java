...
/**
 * 🎭 MockOllamaProvider - Mock implementation of Ollama local LLM provider
 *
 * Simulates Ollama API responses for testing local LLM setups without
 * requiring Ollama to be running or having specific models installed.
 *
...
public class MockOllamaProvider implements MockLlmProvider {
    ...
    /**
     * Creates a mock Ollama provider with default model.
     */
    ...
    /**
     * Creates a mock Ollama provider with specific model.
     *
    ...
    private String generateMockCompletion(final String prompt, final String model) {
        ...
        if (promptLower.contains("code") || promptLower.contains("java")) {
            return """
                    [Mock Ollama - llama2] Code example:

                    public void exampleMethod() {
                        // This is a mock response from local Ollama
                        logger.info("Running locally without API calls");
                    }
                    """;
        }
        ...
        if (promptLower.contains("document") || promptLower.contains("explain")) {
            return """
                    [Mock Ollama - llama2] Local explanation:

                    This component provides functionality for code analysis and documentation
                    generation. It uses local LLM models via Ollama for offline processing.
        ...
        if (promptLower.contains("fix") || promptLower.contains("error")) {
            return """
                    [Mock Ollama - llama2] Suggested improvement:

                    Consider refactoring the implementation:
                    1. Improve error handling patterns
                    2. Add input validation
                    3. Document edge cases

                    This is a mock response demonstrating local LLM usage.
                    """;
        }

        return "[Mock Ollama - " + model + "] Local response: " +
                "This mock response simulates Ollama local inference. " +
                "Actual Ollama server would provide real model outputs.";
    }
}
