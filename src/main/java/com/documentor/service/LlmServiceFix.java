...
/**
 * This class provides a utility method to directly set the LlmService's
 * ThreadLocal configuration. It's designed to work around threading issues
 * where the ThreadLocal value is not being properly propagated to worker
 * threads.
 */
@Service
public class LlmServiceFix {
     ...
     * Call this method before running any LLM operations to ensure the
    ...
    public void setLlmServiceThreadLocalConfig(final DocumentorConfig config) {
        ...
        if (config.llmModels() != null && !config.llmModels().isEmpty()) {
            ...
            LOGGER.warn("Setting ThreadLocal config, but the model list is "
                       + "empty or null");
        }
        ...
        if (verifyConfig != null) {
            LOGGER.info("Successfully set and verified ThreadLocal config "
                       + "with {} models", verifyConfig.llmModels().size());

            // Log that the ThreadLocal config can be expected in child threads
            LOGGER.info("Child threads created by "
                       + "ThreadLocalPropagatingExecutor should now "
                       + "receive this config");
        } else {
            LOGGER.error("Failed to set ThreadLocal config - verification "
                        + "returned null");
        }
    }
    ...
    public boolean isThreadLocalConfigAvailable() {
        ...
        if (config != null) {
            LOGGER.info("ThreadLocal config IS available in thread [{}] "
                       + "with {} models", currentThread.getName(),
                      config.llmModels() != null
