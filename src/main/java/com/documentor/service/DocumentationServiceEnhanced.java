 ...
 *
 * Enhanced version that uses the improved LlmServiceEnhanced components throughout,
...
public class DocumentationServiceEnhanced {
    ...
    public DocumentationServiceEnhanced(
            ...
            final ElementDocumentationGeneratorEnhanced elementDocGeneratorParam,
    ...
    /**
     * 📚 Generates complete project documentation with enhanced error handling and threading
     *
    ...
    public CompletableFuture<String> generateDocumentation(final ProjectAnalysis analysis) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                CompletableFuture<String> mainDocFuture = mainDocGenerator.generateMainDocumentation(analysis)
                ...
                if (config.outputSettings().generateMermaidDiagrams()) {
                    try {
                        CompletableFuture<List<String>> diagramFuture = mermaidDiagramService
                            .generateClassDiagrams(analysis, config.outputSettings().mermaidOutputPath())
                            .orTimeout(DEFAULT_FUTURE_TIMEOUT_SECONDS, TimeUnit.SECONDS)
                            .exceptionally(ex -> {
                ...
                if (config.outputSettings().generatePlantUMLDiagrams()) {
                    try {
                        CompletableFuture<List<String>> plantUMLFuture = plantUMLDiagramService
                            .generateClassDiagrams(analysis, config.outputSettings().plantUMLOutputPath())
                            .orTimeout(DEFAULT_FUTURE_TIMEOUT_SECONDS, TimeUnit.SECONDS)
                            .exceptionally(ex -> {
