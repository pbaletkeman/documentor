...
public class DocumentationFormatter {
     ...
     */
    public void appendHeader(final StringBuilder doc,
            final ProjectAnalysis analysis) {
        doc.append("# ")
                .append(getProjectName(analysis.projectPath()))
                .append("\n\n");
        ...
        if (analysis.projectPath() != null
                && !analysis.projectPath().isEmpty()) {
            doc.append("**Project Path:** `")
                    .append(analysis.projectPath())
                    .append("`\n\n");
        }
    }
     ...
     */
    public void appendStatistics(final StringBuilder doc,
            final ProjectAnalysis analysis) {
        doc.append("## 🔍 Project Statistics\n\n");

        List<CodeElement> elements = analysis.codeElements();
        long classCount = elements.stream()
                .filter(e -> e.type().name().equals("CLASS"))
                .count();
        long methodCount = elements.stream()
                .filter(e -> e.type().name().equals("METHOD"))
                .count();
        long fieldCount = elements.stream()
                .filter(e -> e.type().name().equals("FIELD"))
                .count();

        doc.append("- **Total Elements:** ")
     ...
     */
    public void appendUsageExamples(final StringBuilder doc,
            final ProjectAnalysis analysis) {
        ...
        doc.append("*Usage examples will be generated for key "
                + "components.*\n\n");
    }
     ...
     */
    public void appendApiReference(final StringBuilder doc,
            final ProjectAnalysis analysis) {
