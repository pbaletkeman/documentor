...
class ConfigPackageSimpleCoverageTest {
    ...
    void testBeanUtilsNullValidation() {
        ...
        assertThrows(NullPointerException.class, () ->
            BeanUtils.overrideBean(org.mockito.Mockito.mock(org.springframework.context.ConfigurableApplicationContext.class), null, testConfig)
        );

        assertThrows(NullPointerException.class, () ->
            BeanUtils.overrideBean(org.mockito.Mockito.mock(org.springframework.context.ConfigurableApplicationContext.class), "testBean", null)
        );
    }
    ...
    /**
     * Test ThreadLocalPropagatingExecutorEnhanced constructor with null parameters
     */
    @Test
    ...
    void testCreateExecutorEdgeCases() {
        ...
        assertNotNull(ThreadLocalPropagatingExecutorEnhanced.createExecutor(0, "zero"));
        assertNotNull(ThreadLocalPropagatingExecutorEnhanced.createExecutor(-1, "negative"));

        // Test with null name - covers null name branch
        assertNotNull(ThreadLocalPropagatingExecutorEnhanced.createExecutor(1, null));

        // Test with empty name
        assertNotNull(ThreadLocalPropagatingExecutorEnhanced.createExecutor(1, ""));
    }
    ...
    void testAppConfigCoverage() {
        // Test that AppConfig can be referenced and its methods called indirectly
        assertDoesNotThrow(() -> {
            ...
            assertTrue(methods.length >= 0); // Should have at least some methods
        });
    }
